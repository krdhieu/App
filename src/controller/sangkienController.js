...
let createSangkien = async (req, res) => {
    const [phongban, fields_phongban] = await connectDB.execute(
        'SELECT * FROM phongban '
    );
    const [chucvu, fields_chucvu] = await connectDB.execute(
        'SELECT * FROM chucvu '
    );
...
let addSangkien = async (req, res) => {
    ...
    if (
        ...
        res.status(200).send(
            '<p>Bạn nhập thiếu thông tin <a href="/create-sangkien">Trở về</a></p>'
        );
    }
    let [nhanvien] = await connectDB.execute(
        'select count(*) as "id" from nhanvien where id = ?',
        [manhanvien_1]
    );
    if (nhanvien[0].id == 0) {
        res.status(200).send(
            '<p>Không tồn tại nhân viên <a href="/create-sangkien">Trở về</a></p>'
        );
    }
    let [nguoithamgia] = await connectDB.execute(
        'SELECT count(*) as "id" FROM `nguoithamgia` INNER JOIN sangkien ON nguoithamgia.id_sangkien=sangkien.id WHERE id_nhanvien = ? and id_trangthai = 1 or id_trangthai=2;',
        [manhanvien_1]
    );
    ...
    if (manhanvien_2) {
        let [nhanvien_2] = await connectDB.execute(
            'select count(*) as "id" from nhanvien where id = ?',
            [manhanvien_2]
        );
    ...
    let [dotsangkien] = await connectDB.execute(
        'SELECT id FROM dotsangkien WHERE trangthai = ?',
        [1]
    );
    if (dotsangkien[0] != null) {
        let [id_sangkien_new] = await connectDB.execute(
            'INSERT INTO `sangkien`(`tensangkien`, `id_dot`, `muctieu`, `noidung`, `loiich`, `doituong`) VALUES (?,?,?,?,?,?);',
            [tensangkien, dotsangkien[0].id, muctieu, noidung, loiich, doituong]
        );
        await connectDB.execute(
            'INSERT INTO `nguoithamgia`(`id_nhanvien`, `id_sangkien`, `vaitro`, `tyledonggop`) VALUES (?,?,?,?)',
