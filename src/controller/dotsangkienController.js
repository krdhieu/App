...
let viewDotsangkien = async (req, res) => {
    const [rows, fields] = await connectDB.execute('SELECT * FROM `dotsangkien`');
...
let addDotsangkien = async (req, res) => {
    ...
    await connectDB.execute('INSERT INTO `dotsangkien`(ten, dot, ngaybatdau,ngayketthuc,hannop) VALUES (? , ? , ? ,? ,?)',
        [ten, dot, ngaybatdau, ngayketthuc, hannop]);
    return res.redirect('/quanlydotsangkien')
}

module.exports = {
    viewDotsangkien, addDotsangkien
}